...
<div class="hero-panel">
    <?php if (!empty($flash_message) && is_array($flash_message)): ?>
        <div class="notice <?= htmlspecialchars((string) ($flash_message['type'] ?? 'info'), ENT_QUOTES, 'UTF-8') ?>">
    ...
    <section class="card dashboard-section review-focus-panel">
        <div class="card-header review-focus-header">
            <div>
                ...
                <h2><?= htmlspecialchars((string) ($author_item['title'] ?? 'Untitled'), ENT_QUOTES, 'UTF-8') ?></h2>
                <p>
                    <?= htmlspecialchars((string) (($author_item['book_title'] ?? '') !== '' ? $author_item['book_title'] : 'No book title'), ENT_QUOTES, 'UTF-8') ?>
                    · Imported <?= !empty($author_item['imported_at']) ? htmlspecialchars(date('Y-m-d', strtotime((string) $author_item['imported_at'])), ENT_QUOTES, 'UTF-8') : 'Unknown' ?>
                </p>
            </div>
            <div class="review-focus-actions">
                <span class="status-pill status-<?= htmlspecialchars((string) ($author_item['status_tone'] ?? 'neutral'), ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars((string) ($author_item['status_label'] ?? 'Ready for review'), ENT_QUOTES, 'UTF-8') ?></span>
                <a class="button secondary" href="<?= htmlspecialchars($queueBackUrl, ENT_QUOTES, 'UTF-8') ?>">Back to author queue</a>
            </div>
        </div>
            ...
            <?php if (!empty($author_item['summary']) && is_array($author_item['summary'])): ?>
                <div class="reviewer-metrics" aria-label="Reviewer feedback status summary">
                    <?php foreach ($author_item['summary'] as $metric): ?>
                        <article class="metric-card metric-<?= htmlspecialchars((string) ($metric['tone'] ?? 'neutral'), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <?php if (!empty($author_item['reviews']) && is_array($author_item['reviews'])): ?>
                    <?php foreach ($author_item['reviews'] as $review): ?>
                        <article class="review-note-card">
                            <?php
                                $hasSelectedExcerpt = trim((string) ($review['selected_excerpt'] ?? '')) !== '';
                                ...
                                $reviewTitleRaw = trim((string) ($review['title'] ?? ''));
                                $reviewTitleDisplay = preg_replace('/\breview\s+note\b/i', '', $reviewTitleRaw);
                                $reviewTitleDisplay = is_string($reviewTitleDisplay) ? trim((string) preg_replace('/\s{2,}/u', ' ', $reviewTitleDisplay), " \t\n\r\0\x0B-:;,.()") : '';
                            ?>
                            <div class="review-note-card-header">
                                <div>
                                    <strong><?= htmlspecialchars((string) ($review['reviewer'] ?? 'Reviewer'), ENT_QUOTES, 'UTF-8') ?></strong>
                                    <?php if (!$isRangeNote): ?>
                                        <span class="status-pill status-info">Whole item note</span>
                                    <?php endif; ?>
                                    <?php if ($reviewTitleDisplay !== ''): ?>
                                        <p><?= htmlspecialchars($reviewTitleDisplay, ENT_QUOTES, 'UTF-8') ?></p>
                                    <?php endif; ?>
                                </div>
                                <span class="status-pill status-<?= htmlspecialchars((string) ($review['status_tone'] ?? 'neutral'), ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars((string) ($review['status_label'] ?? 'Open'), ENT_QUOTES, 'UTF-8') ?></span>
                            </div>

                            <dl class="details-list review-note-meta">
                                <dt>Feedback status</dt>
                                <dd>
                                    <span class="status-pill status-<?= htmlspecialchars((string) ($review['feedback_status_tone'] ?? 'neutral'), ENT_QUOTES, 'UTF-8') ?>">
                                        <?= htmlspecialchars((string) ($review['feedback_status_label'] ?? 'Open'), ENT_QUOTES, 'UTF-8') ?>
                                    </span>
                                </dd>
                                <dt>Next step</dt>
                                <dd><?= htmlspecialchars((string) ($review['feedback_status_detail'] ?? 'Review this note and decide how to proceed.'), ENT_QUOTES, 'UTF-8') ?></dd>
                                <dt>Location status</dt>
                                <dd>
                                    <span class="status-pill status-<?= htmlspecialchars((string) ($review['location_status_tone'] ?? 'neutral'), ENT_QUOTES, 'UTF-8') ?>">
                                        <?= htmlspecialchars((string) ($review['location_status_label'] ?? 'Whole item note'), ENT_QUOTES, 'UTF-8') ?>
                                    </span>
                                </dd>
                                <dt>Location detail</dt>
                                <dd><?= htmlspecialchars((string) ($review['location_status_detail'] ?? 'No specific location detail is available.'), ENT_QUOTES, 'UTF-8') ?></dd>
                                <?php if (!empty($review['location_needs_parent'])): ?>
                                    <dt>Fallback context</dt>
                                    <dd><?= htmlspecialchars((string) ($review['location_fallback_context'] ?? 'No fallback context available.'), ENT_QUOTES, 'UTF-8') ?></dd>
                                <?php endif; ?>
                                <dt>Submitted</dt>
                                <dd><?= !empty($review['created_at']) ? htmlspecialchars(date('Y-m-d H:i', strtotime((string) $review['created_at'])), ENT_QUOTES, 'UTF-8') : 'Unknown' ?></dd>
                                <dt>Updated</dt>
                                <dd><?= !empty($review['updated_at']) ? htmlspecialchars(date('Y-m-d H:i', strtotime((string) $review['updated_at'])), ENT_QUOTES, 'UTF-8') : 'Unknown' ?></dd>
                                <dt>Decision</dt>
                            ...
                            <?php if (!empty($review['selected_excerpt'])): ?>
                                <p><strong>Selection:</strong> <?= htmlspecialchars((string) $review['selected_excerpt'], ENT_QUOTES, 'UTF-8') ?></p>
                            <?php elseif (($review['anchor_start_offset'] ?? null) !== null && ($review['anchor_end_offset'] ?? null) !== null): ?>
                                <p><strong>Selection offsets:</strong> <?= (int) $review['anchor_start_offset'] ?>-<?= (int) $review['anchor_end_offset'] ?></p>
                            <?php endif; ?>

                            <?php $reviewDetails = trim((string) ($review['details'] ?? '')); ?>
                            ...
                            <section class="review-feedback-block" aria-label="Reviewer feedback">
                                ...
                                <p class="review-feedback-text"><?= htmlspecialchars($detailsPreview, ENT_QUOTES, 'UTF-8') ?></p>
                            </section>
                            ...
                            <section class="review-feedback-block" aria-label="Resolution context">
                                ...
                                <dl class="details-list review-note-meta">
                                    ...
                                    <dd><?= htmlspecialchars((string) ($review['original_target_context'] ?? 'No original target context available.'), ENT_QUOTES, 'UTF-8') ?></dd>
                                    <dt>Current changed context</dt>
                                    <dd><?= htmlspecialchars((string) ($review['current_changed_context'] ?? 'No current changed context available.'), ENT_QUOTES, 'UTF-8') ?></dd>
                                    <dt>Version linkage</dt>
                                    <dd>
                                        <strong><?= htmlspecialchars((string) ($review['version_linkage_label'] ?? 'No prior link'), ENT_QUOTES, 'UTF-8') ?></strong>
                                        <span><?= htmlspecialchars((string) ($review['version_linkage_detail'] ?? ''), ENT_QUOTES, 'UTF-8') ?></span>
                                    </dd>
                                </dl>
                            </section>
                            ...
                            <div class="action-group" style="margin-top: 0.75rem;">
                                <?php if (($review['status'] ?? '') === 'needs_author_review'): ?>
                                    <?php if (!empty($review['location_needs_parent'])): ?>
                                        <form method="post" action="/dashboard/author/<?= (int) ($author_item['id'] ?? 0) ?>/review/<?= (int) ($review['id'] ?? 0) ?>/resolve" class="review-reparent-form">
                                            ...
                                            <label>
                                                ...
                                                <select name="parent_content_version_id" required>
                                                    ...
                                                    <?php foreach ($authorParentOptions as $parentOption): ?>
                                                        <option value="<?= (int) ($parentOption['id'] ?? 0) ?>"><?= htmlspecialchars((string) ($parentOption['label'] ?? 'Untitled'), ENT_QUOTES, 'UTF-8') ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </label>
                                    ...
                                    <?php else: ?>
                                        <form method="post" action="/dashboard/author/<?= (int) ($author_item['id'] ?? 0) ?>/review/<?= (int) ($review['id'] ?? 0) ?>/resolve">
                                            <input type="hidden" name="_csrf" value="<?= htmlspecialchars((string) ($csrf_token ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                                            <input type="hidden" name="return_queue_query" value="<?= htmlspecialchars($queueQueryString, ENT_QUOTES, 'UTF-8') ?>">
                                            <input type="hidden" name="resolution_action" value="resolved">
